...
from utils.utils_ebad import (
    COCO_BBOX_LABEL_NAMES,
    VOC_BBOX_LABEL_NAMES,
    PM_tensor_weight_balancing_np,
    get_loss_and_success_list,
    model_train,
)
...
def parse_arg():
    ...
    parser.add_argument(
        "--eps", type=int, default=10, help="perturbation level: 10,20,30,40,50"
    )
    parser.add_argument(
        "--iters", type=int, default=10, help="number of inner iterations: 5,6,10,20..."
    )
    parser.add_argument(
        "--root", type=str, default="result", help="the folder name of result"
    )
    parser.add_argument(
        "--victim", type=str, default="RetinaNet", help="victim model"
    )  # RetinaNet
    parser.add_argument("--x", type=int, default=3, help="times alpha by x")
    parser.add_argument(
        "--surrogates", "--names-list", nargs="+", default=["Faster R-CNN", "YOLOv3"]
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="coco",
        help="model dataset 'voc' or 'coco'. This will change the output range of detectors.",
    )
    parser.add_argument("-single", action="store_true", help="only care about one obj")
    parser.add_argument(
        "-no_balancing", action="store_true", help="do not balance weights at beginning"
    )
...
def pre_setup():
    ...
    match dataset:
        case "voc":
            ...
            test_image_ids_path = (
                data_root / "VOCdevkit/VOC2007/ImageSets/Main/test.txt"
            )
...
def main():
    ...
    for model_name in model_list:
        ensemble.append(
            model_train(model_name=model_name, dataset=adv_params["dataset_name"])
        )

    model_victim = model_train(
        model_name=adv_params["victim_name"], dataset=adv_params["dataset_name"]
    )
    ...
    dict_k_valid_id_v_success_list = (
        {}
    )  # lists of success for all mdoels for valid im_ids
    ...
    for im_idx, im_id in enumerate(tqdm(test_image_ids)):
        ...
        if f"{im_id}.jpg" not in dirs["already_run"]:
            ...
            all_categories = set(
                labels.astype(int)
            )  # all apperaing objects in the scene
            ...
            target_pool = list(
                set(range(adv_params["dataset_num_labels"])) - all_categories
            )
            ...
            if adv_params["no_balancing"]:
                ...
                _, LOSS = PM_tensor_weight_balancing_np(
                    im_path,
                    im_np,
                    target,
                    dummy_w,
                    ...
                    adv_params["eps"],
                    n_iters=1,
                    alpha=adv_params["alpha"],
                    dataset=adv_params["dataset_name"],
                )
            ...
            adv_np, LOSS = PM_tensor_weight_balancing_np(
                im_path,
                im_np,
                target,
                w_np,
                ensemble,
                adv_params["eps"],
                n_iters=adv_params["n_iters"],
                alpha=adv_params["alpha"],
                dataset=adv_params["dataset_name"],
            )
            n_query = 0
            loss_bb, success_list = get_loss_and_success_list(
                im_np, adv_np, LOSS, target_clean, all_models
            )
            ...
            if success_list[-1]:
                ...
                while n_query < adv_params["iterw"]:
                    ...
                    adv_np_plus, LOSS_plus = PM_tensor_weight_balancing_np(
                        im_path,
                        im_np,
                        target,
                        w_np_temp_plus,
                    ...
                    loss_bb_plus, success_list = get_loss_and_success_list(
                        im_np, adv_np_plus, LOSS_plus, target_clean, all_models
                    )
                    dict_k_valid_id_v_success_list[im_id].append(success_list)
                    ...
                    adv_np_minus, LOSS_minus = PM_tensor_weight_balancing_np(
                        im_path,
                        im_np,
                        target,
                        w_np_temp_minus,
                    ...
                    loss_bb_minus, success_list = get_loss_and_success_list(
                        im_np, adv_np_minus, LOSS_minus, target_clean, all_models
                    )
                    dict_k_valid_id_v_success_list[im_id].append(success_list)
            ...
            if len(dict_k_sucess_id_v_query) > 0:
                query_list = [
                    dict_k_sucess_id_v_query[key] for key in dict_k_sucess_id_v_query
                ]
                # print(f"query_list: {query_list}")
                print(f"avg queries: {np.mean(query_list)}")
                print(
                    f"success rate (victim): {len(dict_k_sucess_id_v_query) / len(dict_k_valid_id_v_success_list)}"
                )
            ...
            for valid_id in dict_k_valid_id_v_success_list:
                ...
                success_list = (
                    success_list.sum(axis=0).astype(bool).astype(int).tolist()
                )
            ...
            for idx, success_cnt in enumerate(success_list_stack):
                print(
                    f"success rate of {all_model_names[idx]}: {success_cnt / len(dict_k_valid_id_v_success_list)}"
                )
