...
from mmdet_model_info import extra_model_info, model_info
...
def parse_args():
    ...
    parser.add_argument("--data-root", help="custom image file", default="data/coco/")
...
def evaluation(args, config=None, checkpoint=None):
    ...
    # load config
    if config is None:
        config = model_info[args.model_name]["config_file"]
    if checkpoint is None:
        checkpoint = model_info[args.model_name]["checkpoint_file"]
    ...
    cfg.val_evaluator.ann_file = args.data_root + "annotations/instances_val2017.json"
    cfg.test_evaluator.ann_file = args.data_root + "annotations/instances_val2017.json"
...
def evaluation_all(args):
    for model_name in model_info:
        config = model_info[model_name]["config_file"]
        checkpoint = model_info[model_name]["checkpoint_file"]
        print(f"\n\nRUNNING FOR MODEL: {model_name}\n\n")
        evaluation(args, config=config, checkpoint=checkpoint)
